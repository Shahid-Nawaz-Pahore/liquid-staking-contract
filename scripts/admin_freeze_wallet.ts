...
export async function run(provider: NetworkProvider) {
    ...
    ui.write('🧊 Pool Proxy Admin - Freeze Wallet\n');
    ui.write('='.repeat(50) + '\n\n');
    ...
    const walletOwnerStr = await ui.input('Enter wallet owner address to freeze:');
    const walletOwner = Address.parse(walletOwnerStr);

    ui.write('\n📋 Freeze Details:\n');
    ui.write(`   Wallet Owner:    ${walletOwner}\n`);
    ui.write(`   Action:          Freeze wallet (prevent transfers)\n\n`);

    const confirm = await ui.input('Confirm freeze? (yes/no):');
    if (confirm.toLowerCase() !== 'yes') {
        ui.write('❌ Freeze cancelled\n');
        return;
    }

    ui.write('\n🚀 Sending freeze wallet transaction...\n');

    try {
        await pool.sendFreezeWallet(
            sender,
            walletOwner
        );
        ...
        ui.write(`   ${walletOwner} cannot transfer jettons\n`);
